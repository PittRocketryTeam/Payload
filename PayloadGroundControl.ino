...
// There are two signals that can be sent to the rover through serial.
...
void loop()
{
  ...
  while (Serial.available()) {
    ...
    char c = Serial.read();       // Read input from serial (ground computer)
                                  // This can be either 'T' for testing the connection or 'A' for activating the rover
    if (c == 'T') {
      ...
      Serial.read();                  // Empty the serial input buffer if anything other than the two commands are received.
    }
  }
